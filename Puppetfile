# Configuration for librarian-puppet (http://librarian-puppet.com/)
# This installs necessary third-party Puppet Modules for us.

# Install Puppet Tomcat & PostgreSQL modules from PuppetForge
forge "http://forge.puppetlabs.com"
mod "puppetlabs/postgresql"

mod "archive",
  :git => "http://github.com/camptocamp/puppet-archive.git"
mod "tomcat",
  :git => "http://github.com/camptocamp/puppet-tomcat.git"

# Other examples of using librarian-puppet with GitHub
#mod "apt",
#  :git => "git://github.com/puppetlabs/puppetlabs-apt.git"
#
#mod "stdlib",
#  :git => "git://github.com/puppetlabs/puppetlabs-stdlib.git"

# add tomcat
include tomcat::debian

